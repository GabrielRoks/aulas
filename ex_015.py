...
total_votos = (print(f'A quantidade total de fotos é: {votos_cA + votos_cB + votos_cC + votos_brancos + votos_nulos}'))

votos_candidatos = print(f'A porcentagem de votos do candidato A foi de: {(votos_cA * 100) / int(total_votos):.2f}%\n'
                         f'A porcentagem de votos do candiadto B foi de: {(votos_cB * 100) / int(total_votos):.2f}%\n'
                         f'A porcentagem de votos do candiadto C foi de: {(votos_cC * 100) / int(total_votos):.2f}%')

porc_votos_brancos = print(f'A porcentagem de votos brancos é: {(votos_brancos * 100) / total_votos:.2f}%')

porc_votos_nulos = float(print(f'A porcentagem de votos nulos é: {(votos_nulos * 100) / total_votos:.2f}%'))

porc_votos_validos = print(f'A porcentagem de votos válidos é de: {((total_votos - votos_nulos) * 100) / total_votos:.2f}')